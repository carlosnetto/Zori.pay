...
These URLs must be configured in [Google Cloud Console](https://console.cloud.google.com/apis/credentials?project=asdj238sjasd):

```
http://localhost:3000/auth/callback      # React dev (primary)
...
```
1. User clicks "Sign in with Google" on React app (localhost:3000)
   ↓
2. React calls API to get Google auth URL
   - API generates URL with redirect_uri = http://localhost:3000/auth/callback
   ↓
3. Browser redirects to Google OAuth consent screen
   ↓
4. User approves on Google
   ↓
5. Google redirects to: http://localhost:3000/auth/callback?code=xxx&state=yyy
   ↓
